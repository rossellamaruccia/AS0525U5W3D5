...
import com.example.gestioneEventi.payloads.ErrorsDTO;
import com.example.gestioneEventi.payloads.ErrorsWithListDTO;
...
public class ErrorHandler {
    ...
    public ErrorsWithListDTO handleValidationException(ValidationException ex) {
        return new ErrorsWithListDTO(ex.getMessage(), LocalDateTime.now(), ex.getErrorList());
    }
    ...
    public ErrorsDTO handleGenericServerError(Exception ex) {
        ex.printStackTrace();
        return new ErrorsDTO("Mother of all the exceptions!", LocalDateTime.now());
    }
}